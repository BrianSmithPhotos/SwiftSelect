...
because Foundation Models image input is only there; the Mac deployment floor is macOS 15, so the
OS requirement is enforced at runtime via `#available` and the other backends still work below 27. See CLAUDE.md "Hardware & model notes"
